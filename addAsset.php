...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if(empty(trim($_POST["desc"]))){
    ...
    $sql = "SELECT type, description FROM assets WHERE type = ? AND description = ?";
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            if(mysqli_stmt_num_rows($stmt) == 1){
                $desc_err = "Warning!! This description matches with an asset you already own.";
            } else{
  ...
  if(empty($type_err) && empty($desc_err) && empty($value_err)) {
    ...
    $sql = "INSERT INTO assets (type, description, value) VALUES (?, ?, ?)";
             
    if($stmt = mysqli_prepare($link, $sql)) {
      // Bind variables to the prepared statement as parameters
      mysqli_stmt_bind_param($stmt, "ssi", $param_type ,$param_desc, $param_value);
      ...
      // Attempt to execute the prepared statement
      if(mysqli_stmt_execute($stmt)){
        $success = '<div class="alert alert-success"><strong>Succesfully</strong> added new asset. Proceed to Home Page to view </div>';
        $type = false;
        $description = false;
        $value = false;
      } else{
    ...
    <div class="container-fluid">
      <div class="row">
        ...
        <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
          <h2>Fill in the form below to add an asset</h2>
